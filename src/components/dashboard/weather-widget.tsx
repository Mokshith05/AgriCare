...
const WeatherIcon = ({ condition }: { condition: string }) => {
  switch (condition.toLowerCase()) {
    ...
    case 'cloudy':
    case 'partly cloudy':
      return <Cloud className="h-7 w-7 text-gray-300" />;
    case 'rainy':
    case 'showers':
      return <CloudRain className="h-7 w-7 text-blue-400" />;
    case 'thunderstorm':
      return <Zap className="h-7 w-7 text-yellow-300" />;
    default:
      return <Sun className="h-7 w-7 text-yellow-400" />;
  }
};
        ...
        {loading ? (
          ...
          <>
            ...
            {weather.alert && weather.alert.title && (
              <Alert variant="destructive" className="bg-amber-900/50 border-amber-700 text-amber-200">
